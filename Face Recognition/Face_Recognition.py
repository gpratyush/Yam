...
def add() :
	...
	while i <= 25 :
		...
		while True : 		
			...
			if (roi1 is None or (x1 == 0 and y1 == 0 and h1 == 0 and w1 == 0)) :
				continue
			else :
				if (cv2.waitKey(1) & 0xff) == 99 :
					name = "./yalefaces/subject" + str(labels_last) + "." + chr(i+96) +".jpg"
					cv2.imshow("Clicked",roi1)
					cv2.imwrite(name,roi1)
					cv2.waitKey(1000)
					cv2.destroyWindow("Clicked")			
					break;
		i += 1
		cv2.destroyWindow("Picture Click")
def main() :
	...
	while True :
		...
		if (x1 == 0 and y1 == 0 and h1 == 0 and w1 == 0) :
			count=0
			nbr_predicted=0
			conf_sum=0
		...
		if count == 10 :
			...
			elif (nbr_predicted == 0 and count == 10) :
				print "Not Found."
				add()
			count += 1
